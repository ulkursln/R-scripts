...
##Read Data
mydata<-read.delim("D:\\Folders\\doktora\\TIK4\\Analizler\\analysis_for_rawData\\RData\\RData_FaceContactDurationsExtended.txt", sep= " ", header =T)
names(mydata)
...
mydata_withoutOutlier<-mydata[which(mydata$Duration <1933),]
nrow(mydata_withoutOutlier)
boxplot(mydata_withoutOutlier$Duration, horizontal = TRUE)
mydata_withoutOutlier$Duration[order(mydata_withoutOutlier$Duration,decreasing = T)[1:1000]]
...
##Plotted
plot(model_5_2, xlab="Fitted values", ylab = "Residuals" )



model_selected=model_5_2
...
qqp(mydata_withoutOutlier_duration_test$Duration, "lnorm", xlab="lnorm quantiles", ylab = "Duration")
...
# have shown below.
nbinom <- fitdistr(mydata_withoutOutlier_duration_test$Duration, "Negative Binomial")
qqp(mydata_withoutOutlier_duration_test$Duration, "nbinom", size = nbinom$estimate[[1]], mu = nbinom$estimate[[2]], xlab="lnorm quantiles", ylab = "Duration")

# poisson <- fitdistr(mydata_withoutOutlier_duration_test$Duration.t, "Poisson")
# qqp(mydata_withoutOutlier_duration_test$Duration.t, "pois", poisson$estimate)


##Plotted
gamma <- fitdistr(mydata_withoutOutlier_duration_test$Duration, "gamma")
qqp(mydata_withoutOutlier_duration_test$Duration, "gamma", shape = gamma$estimate[[1]], rate = gamma$estimate[[2]], xlab="Gamma quantiles", ylab = "Duration")

#since lmer model robust for non-normal data especially for that much data, we continue with linear resgression

#checked, there is non linear relation
...
PQL2 <- glmmPQL(Duration ~ Role + Gender*PartnerGender+ Role:Gender + Role:PartnerGender, random=(~1+PartnerGender|InterviewerID/IntervieweeID), family = gaussian(link = "log"),
                data = mydata_withoutOutlier_duration_test, verbose = FALSE )
summary(PQL2)

PQL3 <- glmmPQL(Duration ~ Role*Gender*PartnerGender, random=(~1|InterviewerID/IntervieweeID), family = gaussian(link = "log"),
                data = mydata_withoutOutlier_duration_test, verbose = FALSE )
...
interviewer=mydata_withoutOutlier_duration_test[which(mydata_withoutOutlier_duration_test$Role=="Interviewer"),]
nrow_interviewer=nrow(interviewer)
interviewee=mydata_withoutOutlier_duration_test[which(mydata_withoutOutlier_duration_test$Role=="Interviewee"),]
nrow_interviewee=nrow(interviewee)
...
#Duration ~Rolewhen GenderFemale
mydata_withoutOutlier_duration_test_G_F = mydata_withoutOutlier_duration_test[which(mydata_withoutOutlier_duration_test$Gender=="F"),]
meansG_F=tapply(mydata_withoutOutlier_duration_test_G_F$Duration, mydata_withoutOutlier_duration_test_G_F$Role,mean)
sdsG_F=tapply(mydata_withoutOutlier_duration_test_G_F$Duration, mydata_withoutOutlier_duration_test_G_F$Role,sd)
G_F_interviewer=mydata_withoutOutlier_duration_test_G_F[which(mydata_withoutOutlier_duration_test_G_F$Role=="Interviewer"),]
nrow_G_F_interviewer=nrow(G_F_interviewer)
G_F_interviewee=mydata_withoutOutlier_duration_test_G_F[which(mydata_withoutOutlier_duration_test_G_F$Role=="Interviewee"),]
nrow_G_F_interviewee=nrow(G_F_interviewee)
se_G_F_interviewee= sdsG_F[1] / sqrt(nrow_G_F_interviewee)
se_G_F_interviewer= sdsG_F[2] / sqrt(nrow_G_F_interviewer)
...
#Duration ~PartnerGender when RoleInterviewer
mydata_withoutOutlier_duration_test_R_WER = mydata_withoutOutlier_duration_test[which(mydata_withoutOutlier_duration_test$Role=="Interviewer"),]
...
mydata_withoutOutlier_duration_test_R_WER = mydata_withoutOutlier_duration_test[which(mydata_withoutOutlier_duration_test$Role=="Interviewer" & mydata_withoutOutlier_duration_test$Gender=="M" & mydata_withoutOutlier_duration_test$PartnerGender=='M' ),]
meansR_WER_M_M=mean(mydata_withoutOutlier_duration_test_R_WER$Duration)
sdsR_WER=sd(mydata_withoutOutlier_duration_test_R_WER$Duration)
nrow=nrow(mydata_withoutOutlier_duration_test_R_WER)
se_R_WER_M_M= sdsR_WER/ sqrt(nrow)
meansR_WER_M_M
se_R_WER_M_M

mydata_withoutOutlier_duration_test_R_WER = mydata_withoutOutlier_duration_test[which(mydata_withoutOutlier_duration_test$Role=="Interviewer" & mydata_withoutOutlier_duration_test$Gender=="M" & mydata_withoutOutlier_duration_test$PartnerGender=='F' ),]
meansR_WER_M_F=mean(mydata_withoutOutlier_duration_test_R_WER$Duration)
sdsR_WER=sd(mydata_withoutOutlier_duration_test_R_WER$Duration)
nrow=nrow(mydata_withoutOutlier_duration_test_R_WER)
se_R_WER_M_F= sdsR_WER/ sqrt(nrow)
meansR_WER_M_F
se_R_WER_M_F

mydata_withoutOutlier_duration_test_R_WER = mydata_withoutOutlier_duration_test[which(mydata_withoutOutlier_duration_test$Role=="Interviewer" & mydata_withoutOutlier_duration_test$Gender=="F" & mydata_withoutOutlier_duration_test$PartnerGender=='F' ),]
meansR_WER_F_F=mean(mydata_withoutOutlier_duration_test_R_WER$Duration)
sdsR_WER=sd(mydata_withoutOutlier_duration_test_R_WER$Duration)
nrow=nrow(mydata_withoutOutlier_duration_test_R_WER)
se_R_WER_F_F= sdsR_WER/ sqrt(nrow)
meansR_WER_F_F
se_R_WER_F_F
...
meansR_WER_F_M=mean(mydata_withoutOutlier_duration_test_R_WER$Duration)
sdsR_WER=sd(mydata_withoutOutlier_duration_test_R_WER$Duration)
nrow=nrow(mydata_withoutOutlier_duration_test_R_WER)
se_R_WER_F_M= sdsR_WER/ sqrt(nrow)
meansR_WER_F_M
se_R_WER_F_M

##interviewee

mydata_withoutOutlier_duration_test_R_WER = mydata_withoutOutlier_duration_test[which(mydata_withoutOutlier_duration_test$Role=="Interviewee" & mydata_withoutOutlier_duration_test$Gender=="M" & mydata_withoutOutlier_duration_test$PartnerGender=='M' ),]
meansR_WER_M_M=mean(mydata_withoutOutlier_duration_test_R_WER$Duration)
sdsR_WER=sd(mydata_withoutOutlier_duration_test_R_WER$Duration)
nrow=nrow(mydata_withoutOutlier_duration_test_R_WER)
se_R_WER_M_M= sdsR_WER/ sqrt(nrow)
...
mydata_withoutOutlier_duration_test_R_WER = mydata_withoutOutlier_duration_test[which(mydata_withoutOutlier_duration_test$Role=="Interviewee" & mydata_withoutOutlier_duration_test$Gender=="M" & mydata_withoutOutlier_duration_test$PartnerGender=='F' ),]
meansR_WER_M_F=mean(mydata_withoutOutlier_duration_test_R_WER$Duration)
sdsR_WER=sd(mydata_withoutOutlier_duration_test_R_WER$Duration)
nrow=nrow(mydata_withoutOutlier_duration_test_R_WER)
se_R_WER_M_F= sdsR_WER/ sqrt(nrow)
...
mydata_withoutOutlier_duration_test_R_WER = mydata_withoutOutlier_duration_test[which(mydata_withoutOutlier_duration_test$Role=="Interviewee" & mydata_withoutOutlier_duration_test$Gender=="F" & mydata_withoutOutlier_duration_test$PartnerGender=='F' ),]
meansR_WER_F_F=mean(mydata_withoutOutlier_duration_test_R_WER$Duration)
sdsR_WER=sd(mydata_withoutOutlier_duration_test_R_WER$Duration)
nrow=nrow(mydata_withoutOutlier_duration_test_R_WER)
se_R_WER_F_F= sdsR_WER/ sqrt(nrow)
...
mydata_withoutOutlier_duration_test_R_WER = mydata_withoutOutlier_duration_test[which(mydata_withoutOutlier_duration_test$Role=="Interviewee" & mydata_withoutOutlier_duration_test$Gender=="F" & mydata_withoutOutlier_duration_test$PartnerGender=='M' ),]
meansR_WER_F_M=mean(mydata_withoutOutlier_duration_test_R_WER$Duration)
sdsR_WER=sd(mydata_withoutOutlier_duration_test_R_WER$Duration)
nrow=nrow(mydata_withoutOutlier_duration_test_R_WER)
se_R_WER_F_M= sdsR_WER/ sqrt(nrow)
...
# Similarly, separated standard error bars do not mean two values are significantly different. 
# It’s always best to use the appropriate hypothesis test instead. Your eyeball is not a well-defined statistical procedure.


# Create a, b, c, d variables
Duration <- c(555, 873.7, 645.8, 627.5, 672.7, 472.4, 557.9, 581.1 )
...
p + theme(
  plot.title = element_text(color="black", size=20, face="bold.italic"),
  axis.title.x = element_text(color="black", size=20, face="bold"),
  axis.text.x = element_text(color="black", size=0, face="bold"),
  axis.title.y = element_text(color="black", size=20, face="bold"),
  axis.text.y = element_text(color="black", size=18, face="bold"),
  strip.text.x = element_text(size = 20, colour = "black"),
  legend.text=element_text(color="black", size=18, face="bold"),
